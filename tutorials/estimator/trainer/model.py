...
KEY_FEATURE_COLUMN = 'key'  # Dick ??what is it?
...
INPUT_COLUMNS = [
    ...
    tf.feature_column.categorical_column_with_vocabulary_list('dayofweek',
                                                              vocabulary_list=['Sun', 'Mon', 'Tues', 'Wed', 'Thu',
                                                                               'Fri', 'Sat']),
    ...
    # Engineered features that are created in the input_fn (add_engineered)
...
def build_estimator(model_dir, nbuckets, hidden_units):
  ...
  """
    ...
    # Sample only - Represent a 10-element vector in which each cell contains a tf.float32.
    # vector_feature_column = tf.feature_column.numeric_column(key="Bowling", shape=10)
    # Sample only - Represent a 10x5 matrix in which each cell contains a tf.float32.
    # matrix_feature_column = tf.feature_column.numeric_column(key="MyMatrix", shape=[10,5])

    # Dick: Binning
    # Bucketize the lats & lons
    latbuckets = np.linspace(38.0, 42.0, nbuckets).tolist()  # 从38 到42， 分成nubckets段
    lonbuckets = np.linspace(-76.0, -72.0, nbuckets).tolist()
    b_plat = tf.feature_column.bucketized_column(plat, latbuckets)  # pickup latitude 纬度
    b_dlat = tf.feature_column.bucketized_column(dlat, latbuckets)  # dropoff latitude 纬度
    b_plon = tf.feature_column.bucketized_column(plon, lonbuckets)  # pickup longitude 经度
    b_dlon = tf.feature_column.bucketized_column(dlon, lonbuckets)  # dropoff longitude 经度

    # Feature cross (特征交叉)
    ploc = tf.feature_column.crossed_column([b_plat, b_plon], nbuckets * nbuckets)  # pickup localtion
    dloc = tf.feature_column.crossed_column([b_dlat, b_dlon], nbuckets * nbuckets)  # dropoff localtion
    ...
    deep_columns = [
        ...
        # dimension of embedding vector = 10, usually = numOfCategories **0.25
